...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT p.title, (SUM(pl.amount) - p.funding_goal) AS overage FROM projects p JOIN pledges pl ON p.id = pl.project_id HAVING overage >= 0 GROUP BY p.title ORDER BY overage DESC"
end
